...
|---------|----------|-------|
| Fluxer server | ✅ | Built from source (GHCR image is private) |
...
- If enabling voice/video: ports `7881` (TCP), `3478` (UDP), `5349` (TCP), `50000–50100` (UDP) open
...
1. Open firewall ports: `7881/tcp`, `3478/udp`, `5349/tcp`, `50000-50100/udp`
...
Since the server is built from source, re-run `setup.sh` to pull the latest code and rebuild:

```bash
bash setup.sh
# Answer "Yes" to re-run, "No" to keep existing data
```
...
Check that ports `7881/tcp`, `3478/udp`, `5349/tcp`, and `50000-50100/udp` are open. See [DEPLOYMENT.md](DEPLOYMENT.md) for details.
